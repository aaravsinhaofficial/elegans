...
from elegans.experiment.system_utils import (
    capture_system_info,
...
class TestPackageVersion:
    ...
    def test_get_package_version_installed(self):
        ...
        with patch("elegans.experiment.system_utils.version", return_value="1.0.0"):
            ver = get_package_version("test-package")
            assert ver == "1.0.0"

    def test_get_package_version_not_installed(self):
        """Test handling package not found."""
        with patch("elegans.experiment.system_utils.version") as mock_version:
...
class TestTorchVersion:
    ...
    def test_get_torch_version_installed(self):
        ...
        with patch(
            "elegans.experiment.system_utils.get_package_version",
            return_value="2.1.0",
        ):
...
class TestCaptureSystemInfo:
    ...
    def test_capture_system_info_cpu(self):
        ...
        with (
            patch(
                "elegans.experiment.system_utils.get_python_version",
                return_value="3.12.0",
            ),
            patch(
                "elegans.experiment.system_utils.get_torch_version",
                return_value="2.1.0",
            ),
        ):
